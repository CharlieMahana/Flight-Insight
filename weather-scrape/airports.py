...
with_geojson = with_iata_code.with_columns(
    pl.struct(["latitude_deg", "longitude_deg", "elevation_ft"])
    .apply(
        lambda loc: {
            "type": "Point",
            "coordinates": [
                loc["longitude_deg"],
                loc["latitude_deg"],
                loc["elevation_ft"] or 0.0 * 0.3048,
            ],
        }
    )
    .alias("location")
...
airport_data = with_geojson.select(
    pl.col(
        [
            "id",
            "ident",
